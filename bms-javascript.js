...
function authorList(){
    ...
    .then((response) => {
        ...
        for(let eachBook of response){
            data += `<tr>
                        <td>${eachBook.authId}</td>
                        <td>${eachBook.authFirstName}</td>
                        <td>${eachBook.authLastName}</td>
                        <td><button type="button" class="btn btn-danger small" onClick="deleteAuthor(${eachBook.authId})">Remove</button></td>
                    </tr>`;
        }
        data += `</tbody></table></div>`;
        document.getElementById("main-content").innerHTML = data;
    ...
    fetch("http://localhost:7777/api/authors")
        ...
        .then((response) => {
            ...
            <form>
                ...
                <input type="text" class="form-control" id="AuthorId" placeholder="Enter Author Id" name="authId">
                </div>
                <div class="mb-3">
                <label for="AuthorFirstName" class="form-label">Author first name:</label>
                <input type="text" class="form-control" id="AuthorFirstName" placeholder="Enter Author First Name" name="authFirstName">
                </div>
                <div class="mb-3">
                <label for="AuthorLastName" class="form-label">Author last name:</label>
                <input type="text" id="AuthorLastName" class="form-control" placeholder="Enter Author Last Name" name="authLastName">
                </div>
                <button type="button" onClick="addAuthorTo()" class="btn btn-primary">Add Author</button>
...
function addAuthorTo(){
    //alert("addBookTo() called");
    let newAuthor = {
        authId: document.getElementById("AuthorId").value,
        authFirstName: document.getElementById("AuthorFirstName").value,
        authLastName: document.getElementById("AuthorLastName").value
    ...
    .then((res)=>res.json())
    .then((response)=>authorList());
}

function deleteAuthor(authId){
    //alert(bookId);
    // use fetch api and consume the delete endpoint of backend application
    fetch("http://localhost:7777/api/authors/" + authId, {
        method: "DELETE"})
        .then((res) => res.json())
        .then((response) => authorList());
}
